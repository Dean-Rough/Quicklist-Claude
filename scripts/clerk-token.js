...
const frontendApi =
    ...
    getFrontendApi(publishableKey);

if (!frontendApi) {
    logError('Unable to determine Clerk frontend API. Provide CLERK_FRONTEND_API or a valid publishable key.');
}
...
const main = async () => {
    ...
    if (!sessionToken) {
        logError('Clerk did not return a testing token. Check that testing tokens are enabled for your project.');
    }
