...
const userResult = await pool.query('SELECT ebay_auth_token FROM users WHERE id = $1', [userId]);
...
await pool.query('INSERT INTO images (listing_id, image_url, ...) VALUES ($1, $2, ...)', [
  listingId,
  imageUrl,
]);
```
...
```javascript
const imageValues = images.map((img, i) => `($1, $2, $3, $4)`).join(', ');
const imageParams = images.flatMap((img, i) => [listing.id, img.data, i, img.isBlurry || false]);
await pool.query(
