...
```html
<button class="hamburger-btn" onclick="app.toggleMobileMenu()" 
...
```html
<a class="nav-link" href="#photo-tips" onclick="app.navigateTo('photoTips'); return false;">Photo Tips</a>
```

**Line 391-401** - Hero CTA buttons lack keyboard handler
```html
<button class="btn btn-secondary" onclick="document.getElementById('homeView')...">
```
**Fix:** Add `onKeyDown` handler for Enter/Space keys

**Line 531** - Lottie animation containers lack descriptive labels
```html
<div id="step1-animation" class="step-image" aria-label="Animated illustration of scanning..."></div>
```
...
```html
<input type="checkbox" id="imageEnhancement" onchange="app.toggleImageEnhancement(this.checked)" disabled />
```
...
```html
<button class="voice-input-button" type="button" aria-pressed="false" onclick="app.toggleVoiceInput('itemHint', this)">
```
...
```html
<input type="text" class="form-input" id="itemHint" name="item-description"
