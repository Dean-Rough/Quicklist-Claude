...
3. **Color-Only Information** (WCAG 1.4.1 violation)
   ...
   **Fix:**
   ...
   <div class="pricing-card featured">
     <div class="pricing-badge"><svg>...</svg> Most Popular</div>
   </div>
   ```
     ...
     <div class="grid grid-3">
       <div class="card">
         ...
         <section class="section">
           ...
           <div class="grid grid-3">
             <div class="card">
               <h3 class="card-title">AI-Powered Descriptions</h3>
               <!-- ✅ Proper nesting -->
             </div>
           </div>
         </section>
...
1. **Weak Value Proposition Hierarchy**
   ...
   **Hero subtext (line 255-259):**
   ...
   ```html
   <p>
     Stop staring at that overflowing drawer. QuickList AI transforms your photos into professional
     marketplace listings in seconds. Perfect for Vinted, eBay, and Gumtree.
   </p>
   ```
...
2. **CTA Button Ambiguity** (line 268)
   ...
   ```html
   <button class="btn btn-primary" onclick="app.showAuthModal()">Get Started Free</button>
   ```
...
3. **Auth Modal UX** (JavaScript app.js)
   ...
   **Clerk Button Visibility Issue (line 219-225):**
   ...
   ```html
   <button id="signInBtn" class="btn btn-secondary" style="display: none">
     <!-- ❌ HIDDEN BY DEFAULT -->
     Sign In
   </button>
   ```
...
1. **Image Upload Confusion** (index.html line 897-921)
   ...
   **Fix (line 917):**
   ...
   <p class="upload-hint">Add 3-5 photos for best results. Include tags & labels.</p>
   ```
...
2. **Blur Detection UX** (app.js line 394)
   ...
   **Bad:** No user guidance on _fixing_ blur

   **Current feedback (index.html line 910-912):**

   ```html
   ${img.isBlurry ? '
   <div class="image-thumbnail-status warning">⚠️ Blur Detected</div>
   ' : '' }
   ```
   ...
   **Add to app.js after blur detection (line 414):**
   ...
   ```javascript
   if (isBlurry) {
     this.showToast('Photo is blurry. Retake with better lighting?', 'warning', {
       action: 'Retake',
...
3. **Generate Button Disabled State** (index.html line 987)
   ...
   ```html
   <button class="btn btn-primary" id="generateBtn" disabled>Generate Listing</button>
   ```
...
2. **Wizard Progress Indication** (index.html line 1021)
   ...
   **Fix (line 1021):**
   ...
   ```html
   <div class="wizard-step" role="tab" aria-selected="false" aria-label="Step 1: Upload Photos">
     <strong>STEP 1</strong>
     <span>Photos</span>
   </div>
   ```
...
3. **Form Labels** (index.html line 947-953)
   ...
   **Fix:**
   ...
   <input type="text" class="form-input" id="itemHint" name="itemHint" />
   ```
...
1. **CTA Copy Not Benefit-Driven**
   ...
   **A/B Test Recommendation:**
   ...
   <!-- Variant B: Process -->
   <button class="btn btn-primary btn-lg">Upload Photos → Get Listing</button>
   ```
...
<div style="display: flex; align-items: center; gap: 0.5rem">
  ✓ No credit card required ✓ 5 free listings per month ✓ Setup in 60 seconds
</div>
```
...
3. **Security Badge Missing**
   ...
   ```html
   <div class="security-badge">🔒 Secure payment by Stripe</div>
   ```
...
5. **Form: Add Labels to Inputs**
   ...
   - **Fix:**
   ```html
   <label class="form-label" for="itemHint"></label>
   ```
...
8. **UX: Add Upload Guidance**
   ...
   ```html
   <p class="upload-hint">📸 Add 3-5 clear photos. Include labels & tags for best results.</p>
   ```
