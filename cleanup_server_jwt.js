...
if (oauthClientStart !== -1) {
  const oauthClientEnd = lines.findIndex(
    (line, idx) => idx > oauthClientStart && line.includes('const { v4: uuidv4 }')
  );
  if (oauthClientEnd !== -1) {
    removals.push({
      start: oauthClientStart,
      end: oauthClientEnd - 1,
      name: 'Conditional Google OAuth client',
    });
  }
}
...
if (validatePasswordStart !== -1) {
  const validatePasswordEnd = lines.findIndex(
    (line, idx) => idx > validatePasswordStart && line === '}'
  );
  if (validatePasswordEnd !== -1) {
    removals.push({
      start: validatePasswordStart,
      end: validatePasswordEnd + 1,
      name: 'validatePassword function',
    });
  }
}
...
const legacyOAuthStart = lines.findIndex((line) =>
  line.includes('// Legacy Google OAuth endpoints')
);
if (legacyOAuthStart !== -1) {
  ...
  for (let i = legacyOAuthStart; i < lines.length; i++) {
    ...
    if (inOAuthBlock) {
      ...
      if (braceCount === 0 && line.includes('}')) {
        ...
        if (
          i + 1 < lines.length &&
          (lines[i + 1].includes('if (googleClient)') || lines[i + 1].includes('// Legacy'))
        ) {
          inOAuthBlock = false;
          continue;
        } else {
  ...
  if (legacyOAuthEnd !== -1) {
    removals.push({
      start: legacyOAuthStart,
      end: legacyOAuthEnd,
      name: 'Legacy Google OAuth endpoints',
    });
  }
}
...
console.log(`\n✅ Cleanup complete!`);
console.log(
  `📄 New file: ${newLines.length} lines (removed ${lines.length - newLines.length} lines)`
);
