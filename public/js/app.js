        ...
        const app = {
            ...
            setupPWAInstall() {
                ...
                window.addEventListener('beforeinstallprompt', (e) => {
                    ...
                    const installButton = document.getElementById('installPWA') || document.getElementById('pwaInstallButton');
                    if (installButton) {
            ...
            formatDamageType(type) {
                const typeMap = {
                    ...
                    'missing_part': '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--error)" stroke-width="2" style="display:inline-block;vertical-align:middle;margin-right:4px"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>Missing Part',
                ...
                switch ((platform || 'vinted').toLowerCase()) {
                    ...
                    case 'depop': {
                        ...
                        return {
                            ...
                            description: [baseDescription, depopMeta, hashtagKeywords.join(' ')].filter(Boolean).join('\n\n')
                        };
                    }
                    case 'facebook': {
                        ...
                        return {
                            ...
                            description: [cleanTitle, meta, baseDescription, hashtagKeywords.join(' ')].filter(Boolean).join('\n\n')
                        };
                    }
                    case 'gumtree': {
                        return {
            ...
            applyPlatformFormat(platform) {
                ...
                this.displayListing(formatted, this.state.currentListing.pricingIntelligence, this.state.currentListing.stockImageData);
            },
...
${this.state.currentListing?.keywords?.join(', ')}
            ...
            async postToEbay() {
                ...
                try {
                    ...
                    // Update button
                    btn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:inline-block;vertical-align:middle"><polyline points="20 6 9 17 4 12"></polyline></svg> Posted to eBay';
            ...
            async saveListing() {
                ...
                try {
                    ...
                    const listingData = {
                        ...
                        rrp: document.getElementById('outputRRP').value,
                        price: document.getElementById('outputPrice').value,
                        location: document.getElementById('itemLocation')?.value || this.state.currentListing.location || '',
                        keywords: this.state.currentListing.keywords,
                        sources: this.state.currentListing.sources,
            ...
            renderSavedItems() {
                ...
                grid.innerHTML = listingsToRender.map(listing => {
                    ...
                    const createdDate = listing.created_at ? new Date(listing.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : 'Draft';
            ...
            renderDashboard() {
                ...
                if (activityEl) {
                    ...
                    activityEl.innerHTML = items.length ? items.join('') : '<p style="color: var(--text-muted);">No recent activity yet</p>';
                }

                if (tipsEl) {
                    const tipSource = metrics.tips && metrics.tips.length ? metrics.tips : this.state.dashboardTips;
                    tipsEl.innerHTML = tipSource.map(tip => `<li style="color: var(--text-secondary);">• ${tip}</li>`).join('');
                }
            },
            ...
            renderMessages() {
                ...
                list.innerHTML = this.state.messages.map(message => {
                    ...
                    const initials = buyerName.split(' ').map(part => part[0]).join('').slice(0, 2).toUpperCase();
                    const badge = message.unread ? '<span class="badge" style="background: var(--accent-indigo);">Unread</span>' : '';
                    const quickReplies = this.getQuickReplies(message.platform).map(reply => {
            ...
            getQuickReplies(platform = '') {
                ...
                if (normalized.includes('ebay')) {
                    return ['Thanks for reaching out! I can ship today.', 'I can do a small discount for quick payment.'];
                }
            ...
            async loadProfileData() {
                ...
                if (profileAvatar) {
                    if (user.imageUrl || user.avatarUrl) {
                        ...
                        const name = user.fullName || user.name || user.firstName || user.emailAddresses?.[0]?.emailAddress || user.email || 'User';
                ...
                if (profileName) {
                    const name = user.fullName || user.name || user.firstName || user.emailAddresses?.[0]?.emailAddress?.split('@')[0] || 'User';
                ...
                if (memberSince) {
                    ...
                    if (createdAt) {
                        ...
                        memberSince.textContent = date.toLocaleDateString('en-GB', { month: 'short', year: 'numeric' });
                    } else {
            ...
            async showBarcodeScanner() {
                ...
                try {
                    ...
                    this.showToast('Camera access denied. Please enable camera permissions in your browser settings.', 'error');
                    return;
                }
            ...
            manualBarcodeEntry() {
                ...
                const icons = {
                    success: '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:inline-block;vertical-align:middle"><polyline points="20 6 9 17 4 12"></polyline></svg>',
                    error: '✕',
                    warning: '⚠',
                    info: 'ℹ'
                };
